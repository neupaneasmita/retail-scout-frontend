...
function CategoryFilterCheckbox({ name, handleChange, checked = false }) {
  return (
    <>
...
const CategoryFilter = ({ selectedCategories, setSelectedCategories }) => {
  ...
  const [showMore, setShowMore] = useState(categories.length > limit);
  const [checkedItems, setCheckedItems] = useState([]);
  const value = [];
  const cat = useSelector((state) => state.fetchtableList.category_uncheck);
  const categoryData = useSelector((state) => state.fetchtableList.product_categories);

  //Fetch platform
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axiosInstance.get(`/count-category`);
  ...
  const handleChange = (e) => {
    ...
    if (checked) {
      setSelectedCategories({
        ...selectedCategories,
        productCategory: [...selectedCategories.productCategory, filterName],
      });
      dispatch(actions.setCategoryList(filterName));
    } else {
      setSelectedCategories({
        ...selectedCategories,
      ...
      dispatch(actions.filterCategoryList(filterName));
    }
  };

  //Categories Search
  const filteredData = useMemo(() => {
    let recievedData = categories;
    if (categoriesSearch) {
      recievedData = recievedData.filter((item) => {
        return item.product_category.toLowerCase().includes(categoriesSearch.toLowerCase());
      });
    }
    return recievedData;
  ...
  return (
    <div className="mb-8">
      <div className="mb-4">
        <div className="flex justify-between items-center">
          <div className="flex items-center">
            ...
            <div className="link text-secondary">Product Category</div>
          ...
          <div className="flex flex-1 ml-1">
            <div className="relative flex items-center w-full">
              ...
              <input
                placeholder=""
                className="relative bg-transparent focus:outline-none active:border-b-2 w-full active:border-indigo-600 border-transparent border-b-2 focus:border-secondary z-20"
              ...
              <div className="bg-gray-200 bg-opacity-80 transition-all hover:bg-gray-300 hover:bg-opacity-70 w-8 h-7 flex items-center justify-center rounded cursor-pointer " 
                ...
                <ArrowDown
                  className={`stroke-current text-secondary transition-transform duration-500 transform ${showCategories && "-rotate-180"
                    }`}
                />
              </div>
            </div>
      ...
      <div className={`max-h-0 overflow-hidden transition-all duration-500 ${showCategories ? 'max-h-104 ease-in' : 'ease-out'}`}>
        <div
              ...
              >
                <div>
                  <CategoryFilterCheckbox
                    ...
                    checked={checkedItems[item.product_category]}
